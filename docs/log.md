...
- 虚拟滚动按帧合并、缓冲窗口复用，移除滚动过程中的无条件重排和全量响应式遍历。
- 选择状态使用 Set，补齐滚动、resize、keep-alive 和动态模式切换的资源清理。
- 修复请求竞态、同步/异步异常 loading 和销毁后的回填。
- 普通表格减少重排，列数组累积和长段合并避免重复复制及递归溢栈。
- 保持原有 API 和 Element UI 样式；新增真实模板、10 万行滚动及新旧 DOM/CSS 回归测试。
